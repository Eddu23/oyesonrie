/* ==========================================================================
   OYE SONRÍE — script.js
   Reveal (IntersectionObserver) · Parallax · Filtro portafolio ·
   Tabs de paquetes · Contador · Lightbox · Formulario (Formspree) ·
   WhatsApp flotante
   ========================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ------------------------------------------------------------------
     PRELOADER — se oculta en cuanto la foto del hero está lista
     (no espera a la carga completa de la página, pero tampoco se
     oculta a ciegas con un temporizador fijo que podría revelar la
     foto a medio cargar).
  ------------------------------------------------------------------ */
  ...
  if (heroImg) {
    if (heroImg.complete && heroImg.naturalWidth > 0) {
      ...
      heroImg.addEventListener('load', () => setTimeout(hidePreloader, 150));
      heroImg.addEventListener('error', () => setTimeout(hidePreloader, 150));
    }
  } else {
    window.addEventListener('load', hidePreloader);
  }

  // Red de seguridad: nunca dejar el preloader más que unos segundos,
  // por si algo falla en la carga de la imagen.
  ...
  /* ------------------------------------------------------------------
     SCROLL REVEAL — con IntersectionObserver (no ScrollTrigger).
  ...
  function getRevealProfile(el){
    ...
    if (el.classList.contains('testimonial') || el.classList.contains('invite-card')) {
      return {
        from: { opacity: 0, y: 52 },
        to:   { opacity: 1, y: 0, duration: .95, ease: 'power3.out' }
      };
    }
    if (el.classList.contains('about__figure')) {
      return {
  ...
  const REVEAL_GROUP_SELECTOR = '.testimonials__grid, .invites__examples, .masonry, .packages__row';
  ...
  /* ------------------------------------------------------------------
     PARALLAX — se eliminó el efecto sobre la foto del hero: el cálculo
     de ScrollTrigger a veces se desincronizaba justo al cargar la
     página, dejando la imagen "adelantada" (como ya scrolleada) sin
     que el usuario hubiera movido el scroll, revelando el color de
     respaldo detrás de la foto. Mismo tipo de problema que tuvimos
     con el parallax de la foto de "Nosotros".
  ------------------------------------------------------------------ */
  ...
  /* ------------------------------------------------------------------
     PORTFOLIO — "Ver todas las fotos" (solo en móvil, vía CSS)
  ------------------------------------------------------------------ */
  const masonryWrap = document.getElementById('masonryWrap');
  const portfolioExpand = document.getElementById('portfolioExpand');
  const portfolioExpandText = portfolioExpand?.querySelector('.portfolio__expand-text');
  if (masonryWrap && portfolioExpand) {
    masonryWrap.classList.add('is-collapsed');
    portfolioExpand.addEventListener('click', () => {
      const collapsed = masonryWrap.classList.toggle('is-collapsed');
      if (portfolioExpandText) portfolioExpandText.textContent = collapsed ? 'Ver todas las fotos' : 'Ver menos fotos';
      if (collapsed) {
        masonryWrap.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }
    });
  }
  ...
  /* ------------------------------------------------------------------
     PACKAGE TABS — XV años / Bodas / Fiestas, como grupos independientes
  ------------------------------------------------------------------ */
